...
function sendSubscription(subscription, userID) {
  let copiedSubscription = JSON.parse(JSON.stringify(subscription));
  copiedSubscription = { ...copiedSubscription, userID: userID };
  return fetch(`${process.env.REACT_APP_API_URL}/notifications/subscribe`, {
    method: "POST",
    body: JSON.stringify(copiedSubscription),
    headers: {
